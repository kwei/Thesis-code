    ...
    for i, target_i in enumerate(targets):
      for j, target_j in enumerate(targets):
        if not target_i.id == target_j.id:
          ...
          d = pathLossModel.estimateDistance(self._suitablePower[target_i.id-1][target_j.id-1], rss)
          pi = cp.reshape(self._var_p[i][-1][:3],(3,1))
          pj = cp.reshape(self._var_p[j][-1][:3],(3,1))
          soc_constraints.append(cp.SOC( [d] @ ((self._var_SDPt/2)+1), pi - pj))
...
def calculateSDP(groupListSamples, UAVsSamples, pathLossModel, savePath=None):
  ...
  sdpResultSamples = {}
  # pbar = ProgressBar(widgets=widgets, maxval=len(groupListSamples)).start()
